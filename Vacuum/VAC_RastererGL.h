//parte de desenhos de primitivas. aqui nada é 3D, vale lembrar.
#include <GL/gl.h>
#include <GL/glu.h>
  ...
  VAC_Rasterer()
    {
      ...
      iCanvasSize.SetX1(0);
      iCanvasSize.SetY1(0);
      //cout << "rasterer:contrutor padrão"<<endl;
      iBPP=32;
...
//	    glClearDepth( 1.0f );
 ...
 void DisposeShape(VAC_Shape* aShape)
 {
  ...
  inline  void DrawTriangle(BZK_Bitmap*aDestination,VAC_Shape *aShape,VAC_ColorRGBA &aColor,VAC_ColorRGBA &aOutLine)
  { 
...
//	glVertex3f(-1.6+BZK_FastMath::Fix32toReal32(aShape->Point(aShape->iPoint.TotalItems()-1)->GetX())/240.0,1- BZK_FastMath::Fix32toReal32(aShape->Point(aShape->iPoint.TotalItems()-1)->GetY())/240.0,-1.6f);
        for (int counter = 0; counter < aShape->iPoint.TotalItems(); counter++) {
            ...
            glVertex3f(-1.6+BZK_FastMath::Fix32toReal32(point->GetX())/240.0,1- BZK_FastMath::Fix32toReal32(point->GetY())/240.0,-1.6f);
        }
