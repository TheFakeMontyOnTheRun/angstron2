...
///TODO: change stack for something better
	...
	bool SenseClass(VAC_Sector *aStart, int aDirection, BZK_Sint32 aClass) {
		...
		while (current != NULL)// && current->iParent == aStart->iParent)
			if (HasActorClass(current, aClass))
				return true;
			else if (current->Get_Connection(aDirection) > 0)
	...
	virtual void Tick() {
		...
		if (!iNeedRendering && BZK_FastMath::GetDirection(((VAC_Actor *) iPeople->GetActivePlayerPtr())->GetAngle()) !=
							   iCameraLastDirection) {
			iNeedRendering = true;
			iCameraLastDirection = BZK_FastMath::GetDirection(
					((VAC_Actor *) iPeople->GetActivePlayerPtr())->GetAngle());
		}
		...
		for (c = 0; c < iPeople->GetActorsCount(); c++) {
			...
			for (int d = 0; d < iPeople->appearance_class.TotalItems(); d++)
				if (iPeople->appearance_class[d] == ActorClass) //encontrou uma linha cuja classe bate
				{
					...
					if (tmp) {
						...
						if (_Actor->GetAppearance() == NULL ||
							_Actor->GetAppearance()->iResourceName != iPeople->appearance_file[d])
					...
					{
						_Actor->SetAppearance(
								NULL); //faz com que o modelo 3D seja nulo, ja que agora temos uma imagem SVG
						if (_Actor->GetBillBoard() == NULL ||
							_Actor->GetBillBoard()->iResourceName != iPeople->appearance_file[d])
			...
			if (ActorSector < 0) {
				cout << "warning: actor " << c << " has a invalid sector value = " << sector
					 << ". hard setting it to sector 1" << endl;
...
//                _Actor->PushEvent(ThisSector->GetTrigger());
			...
			for (int e = 0; e < iPeople->sensor_myclass.TotalItems(); e++)
				if (iPeople->sensor_myclass[e] ==
					ActorClass) //sendo o caso, passe para uma rotina que detecta uma determinada classe de atores
					if (SenseClass(ThisSector,
								   (iPeople->sensor_direction[e] + BZK_FastMath::GetDirection(_Actor->GetAngle())) % 4,
								   iPeople->sensor_senseclass[e])) {
			...
			if (_Actor->iMoved) {
				...
				if (where != 100) {
					...
					if (FaceConnection == 0 ||
						iWorld->GetSector(FaceConnection)->Get_Connection(BZK_FastMath::GetOpositeDirection(where)) ==
						0) { //ator tendou vazar pela parede. Desfaça o movimento e diga a ele que ele colidiu.
						_Actor->Undo();
						...
						if ((FaceConnection < 0) ||
							(ThisSector->Link != "") && (iPeople->GetActivePlayerPtr()->GetID() == ActorID)) {
							string link;
							if (ThisSector->Link != "")
			...
			for (d = iPeople->GetActorsCount() - 1; d >= 0; d--) {
				...
				if (c != d && playerc->GetCurrentSector() == playerd->GetCurrentSector()
					...
					for (int e = 0; e < iPeople->collision_class.TotalItems(); e++)
						if (iPeople->collision_class[e] == playerc->GetClass() &&
							iPeople->collided_class[e] == playerd->GetClass())
			...
			if (iPeople->GetActorPtr(temp) != NULL && iPeople->GetActorPtr(temp) != iPeople->GetActivePlayerPtr()) {
				...
				if (!dummy->HasBeenDrawn()) {
					...
					if (dummy->GetAppearance() != NULL)
						...
						_iRender->AddToImage(*dummy->GetAppearance(), *dummy->GetPosition(), dummy->GetAngle(),
											 ThisSector->iCandelas + ThisSector->iExtraCandelas, lodlevel);
...
*/
			...
			if (!where) {
				if (OffMod == offmo && (ThisSector->iHint == BZK_LOWER_LEFT || ThisSector->iHint == BZK_LEFT ||
										ThisSector->iHint == BZK_UPPER_LEFT))
					continue;

				if (OffMod == offpo && (ThisSector->iHint == BZK_LOWER_RIGHT || ThisSector->iHint == BZK_RIGHT ||
										ThisSector->iHint == BZK_UPPER_RIGHT))
					continue;

				if (OffMod == CEILING &&
					(ThisSector->iHint == BZK_LOWER_LEFT || ThisSector->iHint == BZK_LOWER_CENTER ||
					 ThisSector->iHint == BZK_LOWER_RIGHT))
					continue;

				if (OffMod == FLOOR && (ThisSector->iHint == BZK_UPPER_LEFT || ThisSector->iHint == BZK_UPPER_CENTER ||
										ThisSector->iHint == BZK_UPPER_RIGHT))
					continue;
			...
			} else if (ThisSector->iForceRender[OffMod])
					...
					if (temp != aActor.GetID()) {
						if (iPeople->GetActorPtr(temp) != NULL) {
							...
							if (!dummy->HasBeenDrawn()) {
								...
								if (dummy->GetAppearance() != NULL)
									_iRender->AddToImage(*dummy->GetAppearance(), *dummy->GetPosition(),
														 dummy->GetAngle(),
														 ThisSector->iCandelas + ThisSector->iExtraCandelas);
								if (dummy->GetCachedBillBoard() != NULL)
									_iRender->AddToImage(
											*dummy->GetCachedBillBoard(),
											*dummy->GetPosition(),
...
*/
				...
				for (int OffMod = 5; OffMod >= 0; OffMod--) {
					...
					if (OffMod == offmo)
						Xoff = -1;
					else if (OffMod == offpo)
						Xoff = 1;
					else Xoff = 0;

					if (OffMod == CEILING)
						Yoff = 1;
					else if (OffMod == FLOOR)
						Yoff = -1;
					else
						Yoff = 0;
