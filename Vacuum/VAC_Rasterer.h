...
#ifdef SUPPORT_SDL_GFX
		vx = (Sint16 *) malloc(
				MAX_VERTEX_PER_POLY * sizeof(Sint16)); //yes, I know its 16 bits, but I like it to be more portable
...
//	cout << "size before:" << aShape->GetNumberOfPoints() << endl;
...
#ifdef SUPPORT_SDL_GFX
		filledPolygonRGBA(aDestination, vx, vy, size, aShape->iFill.r(), aShape->iFill.g(), aShape->iFill.b(),
						  aShape->iFill.a());
#endif
	...
	inline void
	DrawTriangle(BZK_Bitmap *aDestination, VAC_Shape *aShape, int first = 0, int second = 1, int third = 2) {
		VAC_Shape *tmp;
	...
	inline void DrawQuad(BZK_Bitmap *aDestination, VAC_Shape *aShape) {
		...
		cout << "--" << endl;
		for (int c = 0; c < 4; c++) {
			cout << "X:" << (Sint16) BZK_FastMath::Fix32toInt32(aShape->Point(c)->GetX()) << ","
				 << (Sint16) BZK_FastMath::Fix32toInt32(aShape->Point(c)->GetY()) << endl;
		...
		SDL_FillRect(aDestination, &rect,
					 SDL_MapRGBA(aDestination->format, aShape->iFill.r(), aShape->iFill.g(), aShape->iFill.b(),
								 aShape->iFill.a()));
