...
public:
	...
	static VAC_ColorRGBA GetRGBA(BZK_DOMNode *aNode) {
		...
		while (child != NULL) {
			...
			if (BZK_ParsingTools::toLowerCase(nodename) == "_text" &&
				BZK_ParsingTools::TrimToken(child->GetContent()) != "")
	...
	static ConnectionList GetConnections(BZK_DOMNode *aNode) {
		...
		while (child != NULL) {
			...
			if (BZK_ParsingTools::toLowerCase(nodename) == "_text" &&
				BZK_ParsingTools::TrimToken(child->GetContent()) != "")
				for (int c = 0; c < 6; c++)
					con.iConnections[c] = BZK_FastMath::Fix32toInt32(
							BZK_ParsingTools::GetNumber(BZK_ParsingTools::GetSubToken(child->GetContent(), c)));
