...
public:
	...
	void ResetRules() {
		...
		triggered_state.ReleaseAll();; // 9999 (same applies here)
		triggered_trigger.ReleaseAll();; // 9999 (same applies here)
		triggered_newstate.ReleaseAll();; // 9999 (same applies here)

		transition_class.ReleaseAll();; //        class
		transition_state.ReleaseAll();; //          nop
		transition_newclass.ReleaseAll();; //       class (same)

		spawner_class.ReleaseAll();; // class
		spawner_state.ReleaseAll();; // 9999 (any high value - if the user needs this number, just override it)
		spawner_spawnedclass.ReleaseAll();; // class
		spawner_spawnedstate.ReleaseAll();; // class


		droppers_class.ReleaseAll();; // class
		droppers_state.ReleaseAll();; //9999
		droppers_newdroptrigger.ReleaseAll();; //nop

		appearance_file.ReleaseAll();;
		appearance_class.ReleaseAll();;
		appearance_accel_mesh.ReleaseAll();;

		collision_class.ReleaseAll();; // class
		collided_class.ReleaseAll();; // class
		collision_newclass.ReleaseAll();; //9999
...
//		cout <<c<< ":----------------" <<endl;
			...
			for (int e = sound_class.TotalItems() - 1; e >= 0; e--) {
				if (sound_class[e] == ActorClass && sound_action[e] == ActorState) { //se ele é da classe
					...
					else if (PlayerSector == ActorSector)
						channel = PEER_SOUND;
					else
						channel = FAR_SOUND;
					...
					if (triggered_trigger[e] == ActorTrigger) //e estiver droppando trigger
						if (triggered_state[e] == ActorState) //e tiver o determinado estado,
						{
							...
							if (_Actor->iMoved && _Actor->GetAlive()) {
								_Actor->SetCurrentSector(aWorld.GetSectorByPos(*_Actor->GetPosition(), ActorSector,
																			   triggered_newstate[e]));
			...
			for (int e = spawner_class.TotalItems() - 1; e >= 0; e--) {
				...
				if (spawner_class[e] == ActorClass) { //se ele é da classe
					...
					{
						SpawnActor(_Actor->GetAngle(), ActorSector, *_Actor, spawner_spawnedclass[e],
								   spawner_spawnedstate[e]);
						iChanged = true;
					}
...
//				if (droppers_class[e] != BZK_STATIC_ACTOR)
						...
						if (Trigger != 27 && _Actor->iMoved && _Actor->GetAlive()) {
							_Actor->SetCurrentSector(
									aWorld.GetSectorByPos(*_Actor->GetPosition(), ActorSector, Trigger));
							//		std::cout << "atualizou com "<<_Actor->GetCurrentSector() << std::endl;
